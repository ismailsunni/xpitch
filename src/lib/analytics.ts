...
function estimateRole(positional: any, ctx: any, format: FormatKey): any {
  ...
  // Goalkeeper is only distinguishable when a field is defined: u/v are then
  // real-pitch relative, so a keeper reads as central, deep (own goal) and
  // barely moving. Without a field the point cloud is PCA-normalized (extent
  // stretched to fill 0..1), which erases those cues and makes a low-activity
  // outfielder look identical to a keeper — so GK is NOT offered without a
  // field. Even with one, the score is gated on being both central and deep,
  // so a flank or advanced player can never be mistaken for a keeper (central
  // is 0 for wing play, which zeroes the score).
  ...
  const central = (0.2 - Math.min(wide, 0.2)) / 0.2; // 1 = dead centre, 0 = on a flank
  const lowRate = 1 - Math.min(kmPerHour / 7, 1); // 1 = very low distance/hour
  const gkScore = hasFieldRole
    ? (still * 2.4 + deep * 2.6 + defFrac * 1.0 + lowRate * 0.8) * central * Math.min(1, deep * 2 + 0.2)
    : 0;
  ...
  // Add Goalkeeper only when a field makes the signal trustworthy and the
  // gated score clears a real threshold (keeps it out of the list for
  // outfielders instead of showing a bottom-ranked false candidate).
  if (gkScore > 1.2) {
    roles.unshift({ role: format === 'futsal' ? 'Goalkeeper (goleiro)' : 'Goalkeeper', score: gkScore });
  }
